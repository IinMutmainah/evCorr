...
#'
#'\code{tuningCD_rev1} optimizes reliability vector by minimizing Epl using
#' Equation 33 of reference [1] (equivalent of Equation 47 of [2] given for computing beta=1-alpha) with
#' Least square problem by adding epsilon in C[b,k] in zero column in matrix C.
...
# Least square problem with adding epsilon in C[b,k] in zero column in matrix C.
...
tuningCD_rev1 <- function(data,t){
  ...
  # to handle zero column in matrix C in order to be able to learn CD, need epsilon in C[b,k]
  if (any(apply(apply(C, 2, function(x) x==0), 2, all))){
    for(b in 1:ncol(C)){
      for(k in 1:ncol(C)){
        if (all(C[b,]==0) && all(C[,k]==0)) {
          C[b,k] = 0.0000000001
        }
      }
    }
