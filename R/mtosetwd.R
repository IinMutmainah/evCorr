#' Computing a set of partial decision using weak dominance criterion.
#'
#'\code{mtosetsd} computes mass function to make a set of partial decision using weak dominance criterion so called interval dominance.
...
#' @examples
#' m<-c(.1,.2,.5,.3); mtosetwd(m);
#' m<-c(.1,0,.5,.4); mtosetwd(m);


# Computing a set of predictions from m composed of singletons omega_i non dominated using
# the weak dominance criterion, meaning such that it does not exist omega_j with bel(omega_j) >= bel(omega_i)
# and pl(omega_j) >= pl(omega_i) [Universe = Omega = set of acts and 0-1 utilities]
#
# Reference:
...
#            1 if singleton i is a non dominated singleton according to the weak dominance criterion
#            0 otherwise
#
...
mtosetwd <- function(m){
  ...
  if (2^natoms == lm){
    ...
    for (i in 1:natoms){
      for(j in 1:natoms){
        if(i!=j){
          ...
          if(isTRUE(bel[2^(j-1)+1] >= bel[2^(i-1)+1])&&(pl[2^(j-1)+1] >= pl[2^(i-1)+1])){
    ...
    print("ACCIDENT in mtosetwd: length of input vector not OK: should be a power of 2");
  }
}
