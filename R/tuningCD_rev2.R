...
#'
#'\code{tuningCD_rev2} optimizes reliability vector by minimizing Epl using
#' Equation 33 of reference [1] (equivalent of Equation 47 of [2] given for computing beta=1-alpha) with
#' Least square problem by pseudo inverse.
...
#' tuningCD_rev2(data1,t);
#' ## returns
#'  ##     $alpha: 0.2413793 -0.6831683 -1.1951220 ## discount rates alpha should
#'  ##     be [0,1], while by using psedo-inverse the result may be different.
#'  ##     $beta: 0.7586207 1.6831683 2.1951220 ## beta = 1-alpha
#'  ##     $mindist: 2.328521



# Least square problem with pseudo inverse
...
tuningCD_rev2 <- function(data,t){
  ...
  a = t(C)%*%C
  b = t(C)%*%d
  alpha =as.vector(pracma::pinv(a)%*%b)
  #alpha = pinv(C)%*%d

  #alpha=lsqlincon(C,d,lb=0,ub=1);
